...
def data_preparation(config_path,params_path):
    ...
    vocab_size ,max_length,wordtoix,ixtoword=find_vocab_size_max_len(vocab,train_caption)
    lis_of_data=[vocab_size,max_length,wordtoix,train_img,test_img,train_descriptions,ixtoword]
    
    prepaired_data_dirs=config["prepaired_data_dir"]
    for local_dir in (prepaired_data_dirs):
        create_directory([local_dir])
        
    lis_of_data_name=['vocab_size','max_length','wordtoix','train_img','test_img','train_descriptions','ixtoword']     
